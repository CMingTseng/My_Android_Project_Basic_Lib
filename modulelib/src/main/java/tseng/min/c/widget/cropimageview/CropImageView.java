package tseng.min.c.widget.cropimageview;
...
import tseng.min.c.widget.cropimageview.animation.SimpleValueAnimator;
import tseng.min.c.widget.cropimageview.animation.SimpleValueAnimatorListener;
import tseng.min.c.widget.cropimageview.animation.ValueAnimatorV14;
import tseng.min.c.widget.cropimageview.animation.ValueAnimatorV8;
import tseng.min.c.widget.cropimageview.callback.Callback;
import tseng.min.c.widget.cropimageview.callback.CropCallback;
import tseng.min.c.widget.cropimageview.callback.LoadCallback;
import tseng.min.c.widget.cropimageview.callback.SaveCallback;
import tseng.min.c.widget.cropimageview.util.Utils;
...
public class CropImageView extends ImageView {
	...
	private int zoomScalaX = 1;
	private int zoomScalaY = 1;
	...
	private void handleStyleable(Context context, AttributeSet attrs, int defStyle, float mDensity) {
		final TypedArray ta = context.obtainStyledAttributes(attrs, R.styleable.scv_CropImageView, defStyle, 0);
		Drawable drawable;
	...
	private void drawOverlay(Canvas canvas) {
		...
		if (!mIsAnimating
			...
			final PointF circleCenter = new PointF((mFrameRect.left + mFrameRect.right) / 2, (mFrameRect.top + mFrameRect.bottom) / 2);
			final float circleRadius = (mFrameRect.right - mFrameRect.left) / 2;
	...
	private void onMove(int diffX, int diffY) {
		switch (mTouchArea) {
			case CENTER:
	...
	public void setZoom(boolean isZooUp) {
		this.isZoomUp = isZooUp;

		if (mTouchArea == TouchArea.OUT_OF_BOUNDS) {
			mTouchArea = TouchArea.LEFT_BOTTOM;
		}

		if (zoomScalaX == 0 & zoomScalaY == 0) {
			zoomScalaX = 1;
			zoomScalaY = 1;
		}
	...
	private Bitmap decodeRegion() {
		...
		try {
			...
			Log.d(TAG, "An error occurred while cropping the image: " + e.getMessage(), e);
		} catch (OutOfMemoryError e) {
			Log.d(TAG, "OOM Error: " + e.getMessage(), e);
		} catch (Exception e) {
			Log.d(TAG, "An unexpected error has occurred: " + e.getMessage(), e);
		} finally {
	...
	private void saveToFile(Bitmap bitmap, final Uri uri) {
		...
		try {
			...
			Log.d(TAG, "An error occurred while saving the image: " + uri, e);
